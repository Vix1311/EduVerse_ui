...
const Header: React.FC = () => {
  ...
  // When profile changes (after logout can be null), sync isLoggedIn
  useEffect(() => {
    setIsLoggedIn(!!localStorage.getItem('access_token') || !!userProfile);
  }, [userProfile]);

  // If token is deleted elsewhere (other tab/thunk), update automatically
  useEffect(() => {
  ...
  return (
    <div>
      {/* Navbar */}
      <div
        className={`fixed px-1 md:px-20 top-0 left-0 w-full z-50 transition duration-300 ${
      ...
      >
        <div className="mx-auto px-6 py-2 flex items-center justify-between">
          ...
          <Link to="/">
            <div className="flex gap-1 items-center">
              ...
              <span
                className={`text-2xl ml-1 font-semibold ${isScrolled ? 'text-white' : 'text-[#252641]'}`}
              >
                E-Learning
              </span>
            </div>
          ...
          <div className="md:hidden flex items-center gap-4">
            <button
              ...
              className={`text-lg ml-2 transition-colors duration-500 ${isScrolled ? 'text-white' : 'text-[#2F327D]'}`}
            >
              <FaSearch />
            </button>
          </div>
          ...
          <div className="hidden md:flex items-center gap-5 text-gray-800 flex-col md:flex-row ml-auto pr-5 transition duration-300">
            <Link
              to="/course-list"
              className={`hover:text-[#F48C06] font-medium transition-colors duration-500 mr-5 text-lg ${isScrolled ? 'text-white' : 'text-[#252641]'}`}
            >
              Course
            </Link>
            ...
            <div className="flex items-center gap-5">
              <div className="relative inline-flex">
                <WishlistDropdown isScrolled={isScrolled} />
        ...
        {showCategoryBar && window.innerWidth >= 768 && (
          <CategoryBar isScrolled={isScrolled} isInHeader={false} />
        )}
      </div>
    </div>
  );
