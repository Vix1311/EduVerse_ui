...
>('feedback/submitCourseFeedback', async (payload, { rejectWithValue }) => {
  try {
    await axios.post(
      'https://edu-verse-api-rho.vercel.app/api/v1/feedback',
      {
        title: payload.title,
...
>('feedback/fetchAdminFeedbacks', async (params, { rejectWithValue }) => {
  ...
  try {
    const res = await axios.get('https://edu-verse-api-rho.vercel.app/api/v1/feedback', {
      params: {
...
export const deleteFeedback = createAsyncThunk<number, number, { rejectValue: string }>(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await axios.delete(`https://edu-verse-api-rho.vercel.app/api/v1/feedback/${id}`, {
        headers: {
          'X-API-KEY': 'NestjsSuper@Elearning$2025',
          ...authHeaders(),
        },
