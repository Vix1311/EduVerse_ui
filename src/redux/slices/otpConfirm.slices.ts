...
export const verifyEmailOtp = createAsyncThunk(
  ...
  async ({ email, otp }: { email: string; otp: string }, { rejectWithValue }) => {
    try {
      const res = await axios.post('https://edu-verse-api-rho.vercel.app/api/v1/auth/verify-email', {
        email,
        otp,
      });
...
export const resendEmailOtp = createAsyncThunk(
  ...
  async (email: string, { rejectWithValue }) => {
    try {
      const res = await axios.post('https://edu-verse-api-rho.vercel.app/api/v1/auth/resend-otp', { email });
