...
export const fetchUsers = createAsyncThunk<UsersResponse, UsersParams | void>(
  ...
  async params => {
    ...
    const res = await axios.get<UsersEnvelope>('https://edu-verse-api-rho.vercel.app/api/v1/auth', {
...
export const disableUser = createAsyncThunk(
  ...
  async ({ userId, reason }: { userId: string; reason: string }) => {
    ...
    const res = await fetch('https://edu-verse-api-rho.vercel.app/api/v1/admin/disable-user', {
      method: 'POST',
      headers: {
...
export const enableUser = createAsyncThunk('admin/enableUser', async (userId: string) => {
  ...
  const res = await fetch('https://edu-verse-api-rho.vercel.app/api/v1/admin/enable-user', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
  ...
  async (userId: string, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.post(
        'https://edu-verse-api-rho.vercel.app/api/v1/admin/approve-teacher',
        { target_user_id: userId },
        {
...
export const resetUserPassword = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const res = await axios.post(
        'https://edu-verse-api-rho.vercel.app/api/v1/admin/reset-password',
        {
          target_user_id: userId,
  ...
  async (
    ...
    try {
      ...
      const res = await axios.put(
        `https://edu-verse-api-rho.vercel.app/api/v1/auth/${userId}/lock`,
        {
          durationMinutes,
          until,
          reason,
...
export const unlockUser = createAsyncThunk(
  ...
  async (userId: string, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await axios.put(
        `https://edu-verse-api-rho.vercel.app/api/v1/auth/${userId}/unlock`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export const createUserViolation = createAsyncThunk(
  ...
  async (
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await axios.post(
        `https://edu-verse-api-rho.vercel.app/api/v1/auth/${userId}/violations`,
        {
          reason,
          violationType,
          actionTaken,
...
export const fetchTeachers = createAsyncThunk<TeachersListResponse, void>(
  ...
  async () => {
    ...
    const res = await axios.get<TeachersListResponse>(
      'https://edu-verse-api-rho.vercel.app/api/v1/users/teachers',
      { headers: token ? { Authorization: `Bearer ${token}` } : undefined },
    );
...
>('users/changeUserRole', async ({ userId, roleId }, { rejectWithValue }) => {
  try {
    ...
    const res = await axios.put(
      `https://edu-verse-api-rho.vercel.app/api/v1/role/users/${userId}/roles`,
      { roleId },
      { headers: { Authorization: `Bearer ${token}` } },
    );
    return res.data;
...
export const ensureTeacher = createAsyncThunk(
  ...
  async (userId: string, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.post(
        `https://edu-verse-api-rho.vercel.app/api/v1/users/${userId}/ensure-teacher`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
